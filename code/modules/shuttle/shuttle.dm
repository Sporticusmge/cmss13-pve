...
// resistance_flags = RESIST_ALL
	...
	var/obj/effect/abstract/particle_holder/shuttle_dust
...
/obj/docking_port/mobile/proc/create_ripples(obj/docking_port/stationary/S1, animate_time)
	...
	if(uses_dust)
		shuttle_dust = new(S1.loc, /particles/shuttle_dust)
		shuttle_dust.particles.position = generator(GEN_CIRCLE, width*12, height*12, NORMAL_RAND)
		shuttle_dust.particles.width = width * 65
		shuttle_dust.particles.height = height * 65
		addtimer(VARSET_CALLBACK(shuttle_dust.particles, count, 0), SHUTTLE_RIPPLE_TIME - 0.9 SECONDS)
		addtimer(VARSET_CALLBACK(shuttle_dust.particles, spawning, 0), SHUTTLE_RIPPLE_TIME - 0.9 SECONDS)
	return TRUE

/obj/docking_port/mobile/proc/remove_ripples()
	QDEL_LIST(ripples)
	QDEL_NULL(shuttle_dust)
